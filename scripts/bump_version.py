"""Bump the bamboo-mcp-services version string across all relevant files.
...
Note
----
Agents read the installed version at runtime via ``importlib.metadata`` rather
than from a hardcoded ``__version__`` constant, so no source file other than
``pyproject.toml`` needs to be touched.

Inactive (commented out — update manually when/if a ``__version__`` constant
is added to the package, or when additional sub-packages are introduced):

- ``src/bamboo_mcp_services/__init__.py``  — ``__version__ = "X.Y.Z"``
"""
...
_ACTIVE_FILES: list[tuple[str, str, str]] = [
    ...
    # Inactive — uncomment if a __version__ constant is added to __init__.py:
    # (
    #     "src/bamboo_mcp_services/__init__.py",
    #     '__version__ = "{old}"',
    #     '__version__ = "{new}"',
    # ),
]
...
def main() -> None:
    ...
    if len(sys.argv) != 3:
        ...
        print("example: python scripts/bump_version.py 0.1.0 0.1.1", file=sys.stderr)
        sys.exit(1)
